...
class Hyperparameters:
    data_path = os.environ.get("DATA_PATH", "./data/fineweb10B/datasets/fineweb10B_sp4096")
    train_files = os.path.join(data_path, "fineweb_train_*.bin")
    val_files = os.path.join(data_path, "fineweb_val_000000.bin")
    tokenizer_path = os.environ.get("TOKENIZER_PATH", "./data/fineweb10B/tokenizers/fineweb_4096_bpe.model")
...
def mixed_quantize_svd_664(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        if t.ndim == 2 and t.numel() > 65536:
            U, S, V = torch.linalg.svd(t.float(), full_matrices=False)
            rank = 14
            A = U[:, :rank] * torch.sqrt(S[:rank])
            B = V[:rank, :] * torch.sqrt(S[:rank]).unsqueeze(1)
            C = t.float() - (A @ B)
            qA, sA = quantize_intN_per_row(A, clip_range=31) # 6-bit
            qB, sB = quantize_intN_per_row(B, clip_range=31) # 6-bit
            qC, sC = quantize_intN_per_row(C, clip_range=7)  # 4-bit
...
def dequantize_svd_664(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if info in ("passthrough", "passthrough_ctrl", "passthrough_fp16"):
            t = result[name]
            out[name] = t.to(orig_dtype) if t.dtype == torch.float16 else t
            continue
        if isinstance(info, dict) and info.get("type") == "svd_664":
            qA, sA = result[name + ".A.q"], result[name + ".A.s"]
            qB, sB = result[name + ".B.q"], result[name + ".B.s"]
            qC, sC = result[name + ".C.q"], result[name + ".C.s"]
            A = qA.float() * sA.float().view(-1, 1)
            B = qB.float() * sB.float().view(-1, 1)
            C = qC.float() * sC.float().view(-1, 1)
            out[name] = (A @ B + C).to(orig_dtype)
        else:
...
def main() -> None:
    ...
    quant_result, quant_meta = mixed_quantize_svd_664(sd_cpu)
    ...
    if master_process:
        ...
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model svd_664+{_COMPRESSOR}: {quant_file_bytes} bytes")
    ...
    deq_state = dequantize_svd_664(quant_state["w"], quant_state["m"], sd_cpu)
    ...
    if master_process:
        ...
        log0(f"Final Evaluation Time:    {1000.0 * (time.perf_counter() - t_qeval)/1000.0:.2f} s")
        log0("="*80)
